...
def decode_snooz(snooz, decoded_file_path):
  ...
  """
    ...
    decoded_file = open(decoded_file_path, "wb")
    ...
    if version == 1:
        ...
        return decode_snooz_v2(decompressed, last_timestamp_ms, decoded_file)


def decode_snooz_v1(decompressed, last_timestamp_ms, decoded_file):
    """
  ...
  """
    ...
    while offset < len(decompressed):
        ...
        first_timestamp_ms += delta_time_ms
        offset += 7
        decoded_file.write(struct.pack('>II', length, length))
        decoded_file.write(struct.pack('>II', type_to_direction(type), 0))
        decoded_file.write(struct.pack('>II', (first_timestamp_ms >> 32), (first_timestamp_ms & 0xFFFFFFFF)))
        decoded_file.write(type_to_hci(type))
        decoded_file.write(decompressed[offset:offset + length - 1])
        offset += length - 1


def decode_snooz_v2(decompressed, last_timestamp_ms, decoded_file):
    """
  ...
  """
    ...
    while offset < len(decompressed):
        ...
        first_timestamp_ms += delta_time_ms
        offset += 9
        decoded_file.write(struct.pack('>II', packet_length, length))
        decoded_file.write(struct.pack('>II', type_to_direction(snooz_type), 0))
        decoded_file.write(struct.pack('>II', (first_timestamp_ms >> 32), (first_timestamp_ms & 0xFFFFFFFF)))
        decoded_file.write(type_to_hci(snooz_type))
        decoded_file.write(decompressed[offset:offset + length - 1])
        offset += length - 1
    ...
    for line in iterator:
        if found:
            if line.find('--- END:BTSNOOP_LOG_SUMMARY') != -1:
                decode_snooz(base64.standard_b64decode(base64_string), dst_file_path)
                sys.exit(0)
